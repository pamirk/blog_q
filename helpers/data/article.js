 ...
 */
const getPromotedTaxonomy = ( { project, series, show, guide, obsession } ) => {
	if ( project?.id ) {
		return {
			...project,
...
export const getArticleProps = node => {
	...
	let { dateGmt, featuredImageSize, trackingUrls } = post;
	...
	const promotedTaxonomy = getPromotedTaxonomy( { project, series, show, guide, obsession } );
	...
	const ad = {
		...
		targeting: {
			...
			peer39: brandSafety.map( term => `${term}` ), // targeting params must be strings!
			premium: !!guide.slug,
		...
		flexSlotSize: [
			...
			{ viewport: [ 1200, 0 ], slot: [ [ 1600, 521 ], [ 2000, 2000 ] ] },
		],
		engageSlotSize: [
			{ viewport: [ 0, 0 ], slot: [ [ 600, 431 ] ] },
			{ viewport: [ 768, 0 ], slot: [ [ 1400, 521 ] ] },
			{ viewport: [ 1200, 0 ], slot: [ [ 1600, 521 ] ] },
		],
	};
	...
	// If there is no social image, try and fall back to the featuredImage. Guard
	// against socialImage being defined but set to empty string.
	let socialImage = 'string' === typeof post.rawSocialImage && post.rawSocialImage;
	if ( ! socialImage && post.featuredImage?.sourceUrl ) {
