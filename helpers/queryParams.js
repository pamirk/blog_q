...
export const isEmailOffer = ({utm_source, utm_medium, utm_campaign}) => {
    const utm_sources = [
        'from.flipboard.com',
        'getpocket.com',
        'smart.news.com',
...
const isLandingPageMembershipPromo = ({utm_term, utm_content}) => {
...
export const getQueryParamData = ({isLoggedIn, isMember}) => {
    const {
        code,
        email,
        grsf_email,
...
export function getUtmQueryParams() {
    ...
    return utms.reduce((acc, utm) => {
        if (queryParams[utm]) {
            Object.assign(acc, {[utm]: queryParams[utm]});
        }
