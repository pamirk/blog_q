import {MEMBERFUL_PLAN_IDS, allPlans} from '../config/membership';
import {roles, roleProperties, defaultRole} from '../config/users';
import {daysFromToday, parseDateGmt} from './dates';
import {encodeRelayId} from './graphql';
import {get, valueOrDefault} from './utils';
import {isValidEmail, isValidLengthPassword, isValidCharactersPassword} from './validation';
import {
    CANCEL_SUBSCRIPTION,
    CHANGE_PLAN,
    DELETE_ACCOUNT,
 ...
 * @return { String }
 */
export const formatPrice = ({decimal = true, price}) => decimal ? (price / 100).toFixed(2) : price.toLocaleString();
...
export const getUserSetting = (user = {}, setting, language) => {
    ...
    const {amount, currency, id: planId} = plan;
    const {duration, amountOff, percentOff} = get(subscription, 'discount.coupon', {});

    // Is this a decimal currency?
    const {priceDec} = allPlans[planId] || {};
    ...
    switch (setting) {
        case CREDIT_CARD_SUMMARY:
            return valueOrDefault(
                ...
                'unknown',
                'ja' === language
                    ? ({brand, expMonth, expYear, last4}) => `末尾4桁が${last4}の${brand}: ${expMonth}/${expYear}`
                    : ({brand, expMonth, expYear, last4}) => `${brand} ending in ${last4}, exp: ${expMonth}/${expYear}`
            );
        ...
        case MEMBERSHIP_DAYS_LEFT:
            return valueOrDefault(membership.currentPeriodEnd, 'unknown', (date) => daysFromToday(parseDateGmt(date)));
        ...
        case PLAN_PRICE:
            ...
            return valueOrDefault(amount, 'unknown', amount => formatPrice({price: amount, decimal}));
