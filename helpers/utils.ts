...
import {VideoPartsFragment} from '@quartz/content';
 ...
 * @return {void}
 */
export const postMessage = ({action, data, frameEl, origin}) => {
    const message = {
        action,
        data,
        fromId: 'QZParent',
...
export const pick = (keys, obj, init = {}) =>
    keys.reduce((acc, cur) => obj[cur] ? {[cur]: obj[cur], ...acc} : acc, init)
;
...
/**
 * Determines a video’s embed URL given a `VideoPartsFragment` object from our CMS.
 */
export function videoEmbedURL(video?: VideoPartsFragment | null) {
    if (!video) {
        return null;
    }
    switch (video.type?.toLowerCase()) {
        case 'vimeo':
            return `https://player.vimeo.com/video/${video.id}?app_id=122963`;
        default:
            return `https://www.youtube.com/embed/${video.id}?autoplay=0&showinfo=0&modestbranding=1&playsinline=true`;
    }
}
