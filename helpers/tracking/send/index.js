...
import {GTM, VENT} from '../../../helpers/types/tracking';
...
const processQueue = () => {
    ...
    queue.splice(0)
        .forEach(({event, type}) => {
            const {mapUserData, send} = eventHelpers[type];
...
if ('object' === typeof window) {
    setTimeout(() => {
        Object.assign(user, {hasLoadedUserData: true, hasLoadedUserPreferences: true});
        processQueue();
    }, 5000);
 ...
 * @return {undefined}
 */
const index = (type, event, appState = {}) => {
    ...
    const {enqueue} = eventHelpers[type];
