...
import {getQueryParams, isAppArticle} from '../../../../helpers/urls';
import {resetAll, resetEphemeral} from './reset';
import {redactEmailAddressesFromPath} from '../../../../helpers/text';
...
function getMemberDuration(membership) {
    ...
    return createdDate ? daysFromToday(parseDateGmt(createdDate)) : null;
}
...
function getPayingMemberDuration(membership, memberDuration) {
    ...
    return trialEnd ? daysFromToday(parseDateGmt(trialEnd)) : memberDuration;
}
...
function getStateVariables(appState) {
    ...
    if (started.length) {
        [{experimentId, variant: experimentVariant}] = started;
    }
 ...
 */
const universal = (payload, appState) => {

    const {data, pageType, ...rest} = payload;
