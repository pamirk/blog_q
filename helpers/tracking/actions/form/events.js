...
import { uppercaseFirstLetter as uc } from '../../../../helpers/text';
import { GTM } from '../../../../helpers/types/tracking';
...
const transformMembershipData = ( formName, planId, stageName, trialDuration, listSlugs ) => {
	if ( MEMBERSHIP_SIGNUP !== formName || !planId || !allPlans[ planId ] ) {
		return {};
	}

	const { priceInt: membershipPrice, interval: membershipPackage } = allPlans[ planId ];
	...
	if ( listSlugs ) {
		membershipInfo.ListId = listSlugs.join( '|' );
	}
...
const deprecatedGetFieldData = ( action, { context, formName, moduleName }, { fieldName } ) => ( {
	[ GTM ]: {
...
export const deprecatedGetFieldEvent = ( action, { context, formName, moduleName }, { fieldName }  ) => {
	...
	return deprecatedGetFieldData( action, { context, formName, moduleName }, { fieldName } );
};

// Map our tracking data to a GTM event.
export const getFieldEvent = ( { action, context, formName, moduleName, fieldName } ) => {
	const eventSignature = `${action}+${formName}+${moduleName || fieldName}+${context}`;
...
export const getFormEvent = ( action, { context, formName, stageName = '', trialDuration, listSlugs, planId } ) => ( {
	[ GTM ]: {
		event: formName,
		eventCategory: eventCategories[ formName ],
		eventAction: `${uc( action )} ${stageName} form`.replace( / +/g, ' ' ), // remove extra spaces
		eventLabel: context,
		...transformMembershipData( formName, planId, stageName, trialDuration, listSlugs ),
	},
} );
...
export const getCodeValidationEvent = ( trackingData ) => {
	...
	const { code, giftCode, couponCode } = trackingData;
