...
interface QZWindow extends Window {
	QZ?: any;
}
...
try {
	const { country, region } = JSON.parse( ( window as QZWindow ).QZ.geo );
	const countryCode = country.toLowerCase();
	const regionCode = region.toLowerCase();

	Object.assign( userGeoData, { countryCode, regionCode } );
	...
	delete ( window as QZWindow ).QZ.geo;
} catch ( err ) {}
