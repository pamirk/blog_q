...
import {updatePendingRequest, updateUserData} from '../actions/auth';
import {generateRandomPassword, getUserSetting, userHasPermission} from '../../user';
import {get} from '../../utils';
import {getErrorMessage} from '../../errors';
import {USER_EMAIL, USER_NAME} from '../../types/account';
import {withClientSideUserData, withNotifications} from '../../../helpers/wrappers';
import {CREATE_GIFT_REQUEST_ID} from './config';
import {getUserGeoData} from '../../geo';
...
const mapPlanChangeDataToState = data => ({planChangePreview: data});
 ...
 */
const index = (options = {}) => WrappedComponent => {
    ...
    const {useSettings = false} = options;
    ...
    class ComponentWithUserApi extends Component {
        ...
        componentDidMount() {
            ...
            const {countryCode} = getUserGeoData();
        ...
        componentDidUpdate() {
            ...
            if (this.state.countryCodeOverride !== countryCodeOverride) {
                this.setState({countryCodeOverride});
            }
        }
            ...
            return new Promise((resolve, reject) => {
                this.setState({error: null}, () => {
                    fetch(`/api/user/${endpoint}`, fetchOptions)
                        ...
                        .then(data => {
                            const state = {...newState, ...successState, ...mapDataToState(data)};
                            this.updateUserData(data, state);
                            onSuccess(data);
                            resolve(state);
                        })
                        .catch(error => {
                            reject(this.updateUserData({}, {error: getErrorMessage(error)}));
                        });
                });
            });
        }
        ...
        createGift({
            ...
            const {history, notifyError} = this.props;
        ...
        getPlanChangePreview({newPlanId}) {
        ...
        getUserSettings() {
            ...
            this.callApi({endpoint: 'settings'})
                .catch(() => {
                    // Do nothing
                });
        }

        createSocialConnection({idToken, requestId, displayName}) {
        ...
        deleteSocialConnection({provider, confirm, requestId, displayName}) {
            const {notifySuccess, notifyError} = this.props;
        ...
        loginUserWithAuth0Token({idToken}) {
            const {notifyError} = this.props;
        ...
        requestResetPassword({email}) {
            this.callApi({
                ...
                .finally(() => this.setState({initialPassword: 'RESETTING'}));
        }
        ...
        updateCreditCard({onSuccess, tokenObject}) {
        ...
        updateEmailWithVerification({email, password, ...userProps}) {
            this.callApi({
        ...
        updatePassword({password, newPassword, requestId}) {
            const body = {password, newPassword};
            const newState = {initialPassword: null};
        ...
        updateUserData({contentIds, tokens, user, userRole, acceptPromotionalOffer}, newState) {
        ...
        render() {
            ...
            const {planChangePreview, countryCode, countryCodeOverride} = this.state;
