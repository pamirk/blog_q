 ...
 *
 */
const withVisibility = (options = {}) => WrappedComponent => {
    ...
    class ComponentWithVisibility extends Component {
        ...
        shouldComponentUpdate(nextProps, nextState) {
            const {hasDeterminedInitialVisibility, visible} = this.state;
        ...
        subscribe() {
            if (!this.el) {
                console.error('An el is required for withVisibility to work. Please use the setRef prop on this component:', {WrappedComponent});
                return;
            }
        ...
        render() {
            const {hasDeterminedInitialVisibility, visible} = this.state;
            const {lazyLoad, amp} = this.props;

            return (
                <WrappedComponent
                    setRef={this.setRef}
                    hasDeterminedInitialVisibility={hasDeterminedInitialVisibility}
                    resetVisibility={this.reset}
