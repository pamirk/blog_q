...
const WC = () => WrappedComponent => {
    class FieldState extends Component {
        ...
        handleBlur() {
            const {fieldName, fieldState} = this.props;

            this.props.handleFieldChange(fieldName, {...fieldState, blurred: true});
        }

        handleFieldChange(value) {
            const {fieldName, fieldState} = this.props;

            this.props.handleFieldChange(fieldName, {...fieldState, value});
        }

        render() {
            const {fieldState, hint} = this.props;
            const {errors, value, blurred} = fieldState;
            ...
            return (
                <WrappedComponent
                    ...
                    errors={errors}
                    value={value}
                    subtext={showErrors ? errors[0] : hint}
