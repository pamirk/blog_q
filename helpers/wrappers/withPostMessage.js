...
import { get, postMessage } from '../utils';
...
const globalPostMessageListener = evt => {
	...
	handlersToCall.forEach( handler => {
		...
		if ( 'object' === typeof reply && frameEl ) {
			...
			postMessage( { action, data: reply, frameEl, origin } );
		}
	} );
};
...
const withPostMessage = ( WrappedComponent ) => {
	...
	class PostMessageComponent extends Component {
		...
		_addMessageListener( { eventHandler, eventName, frameId } ) {
		...
		render() {
			return (
				<WrappedComponent
					{...this.props}
					addMessageListener={this._addMessageListener}
					removeMessageListener={this._removeMessageListener}
				/>
			);
		}
