...
import { apiFetchOptions, referrers } from '../../config/http';
...
import { getQueryParamData, getUtmQueryParams } from '../../helpers/queryParams';
import { setPaywallStrategy } from '../../helpers/tracking/actions';
...
const useArticlePaywallState = ( article, qualifiers ) => {
	...
	const { promotionalContentIds } = useClientSideUserData();
	...
	} = getQueryParamData( { isLoggedIn, isMember } );
	...
	const setAndLog = useCallback( ( type, data ) => {
		setPaywallType( type );
		log( 'paywall', { id, ...data }, type );
	}, [ id, setPaywallType ] );

	const dispatch = useDispatch();

	useAppMessage( { hasPaywall: !! paywallType } );
	...
	useEffect( () => {
		...
		getDynamicPaywallDecision( id ).then( data => {
			const { conditions, experiment, paywall, strategy, version } = data;
			const newPaywallType = paywall ? PAYWALL_DYNAMIC : null;
			const actionPayload = { conditions, experiment, paywallType: newPaywallType, strategy, version };
