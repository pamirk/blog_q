...
import { Redirect301Error } from 'helpers/errors';
...
function PageChrome ( props: ChromeProps ) {
    ...
    return (
        <div
            // className={cx( 'container', showMarquee && marqueeAdUnit, showMarquee && 'marquee-default' )}
...
export default function Page ( props: {
    ...
    const { edition, isInApp, isAmp } = usePageVariant();
    const { redirectCanonical = true, showMarquee = false } = props;

    // if ( !! redirectCanonical ) {
    //     // We probably don't need to do this at all if a rel=canonical tag is properly set, but in the
    //     // interest of being conservative, we'll provide a lever for disabling the redirect instead of removing it.
    //     // location.pathname can contain unicode characters that wouldn’t match the output of
    //     // `encodeURIComponent`; so we need to test against both `canonicalPath` and a decoded version.
    //     // For example: https://qz.com/search/😄/
    //     const canonicalWithURIDecodedComponents = props.canonicalPath.split( '/' ).map( decodeURIComponent ).join( '/' );
    //     if ( location.pathname !== props.canonicalPath && location.pathname !== canonicalWithURIDecodedComponents ) {
    //         // If this isn’t the canonical path, redirect.
    //         throw new Redirect301Error( props.canonicalPath );
    //     }
    // }
