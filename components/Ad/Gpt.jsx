...
export class Gpt extends React.Component {
	...
	configureSlot() {
		...
		googletag.cmd.push( () => {
			...
			// Add a 'skipHB' targeting parameter for ads that are NOT on an article
			// page. This allow ad ops to anti-target header-bidding.
			if ( ! /\/(article)$/.test( adUnitPath ) ) {
				this.slot.setTargeting( 'skipHB', '1' );
			}
...
export default compose(
	withVisibility( { persistent: true, rootMargin: '200px 0' } ) // load ads earlier in scroll to try and avoid jank
)( Gpt );
